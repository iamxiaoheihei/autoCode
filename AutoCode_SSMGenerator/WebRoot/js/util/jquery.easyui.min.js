 ...
 * 
 */
var EasyUiAjaxCount = 0;//增加easyui的ajax计数器
(function($){
var _1=false;
function _2(e){
...
//edit by gm 当页面局部加载时可能linkbutton还未加载完成
if(_54 == undefined){return false;}
if(_53){
...
},setText:function(jq,texts){
//edit by gm 增加setText的方法，用于改变btn中的text的显示内容
return jq.each(function(){
var str = texts + "<span class=\"m-btn-downarrow\">&nbsp;</span>";
$.data(jq[0],"linkbutton").text = str;
$(jq[0]).find('.l-btn-text').html(str);
});
},disable:function(jq){
return jq.each(function(){
_51(this,true);
...
_132(_89,this);
//edit by gm 增加树节点单击时展开/收缩子节点的逻辑
var _8c=$(this);
_f7(_89,_8c[0]);
...
if(d>3){
//edit by gm 在树节点拖动时会出现$(this).draggable("proxy") = null的情况
...
//edit by gm 树节点移动时，会产生$(_9a).draggable("proxy") == null
if(!$(_97).draggable("proxy")){return false;}	
var _98=_97.pageY;
...
//edit by gm 树节点移动时，会产生$(_9a).draggable("proxy") == null
if(!$(_9a).draggable("proxy")){return false;}
$(_9a).draggable("proxy").removeClass("tree-dnd-yes").addClass("tree-dnd-no");
$(this).removeClass("tree-node-append tree-node-top tree-node-bottom");
},onDrop:function(e,_9b){
//edit by gm(增加onBeforeDrop方法，当return=false时，跳出该方法)
var point;
if($(this).hasClass("tree-node-append")){
point = "append";
}else if($(this).hasClass("tree-node-top")){
point = "top";
}else{
point = "bottom";
}
var ret = _93.onBeforeDrop.call(_92,this,_9b,point);
if(ret == false){
	$(this).removeClass("tree-node-append tree-node-top tree-node-bottom");
	return false;
}
...
function _a3(){
//edit by gm 增加isShadow属性，当isShadow=true时，移动后节点后不删除原有节点ops取原tree上的节点
var _a4;
var ops = $.data($(_a1).parents('ul[id]:first')[0],"tree").options;
if(ops.isShadow){
_a4=$(_92).tree("getNode",_a1);
}else{
_a4=$(_92).tree("pop",_a1);
}
...
//edit by gm 增加isShadow属性，当isShadow=true时，移动后节点后不删除原有节点ops取原tree上的节点
var _a9;
var ops = $.data($(_a5).parents('ul[id]:first')[0],"tree").options;
if(ops.isShadow){
_a9=$(_92).tree("getNode",_a5);
}else{
_a9=$(_92).tree("pop",_a5);
}
_a8.data=_a9;
$(_92).tree("insert",_a8);
...
//edit by gm 增加判断this传入对象为page对象是直接返回
...
//edit by gm 根据路径查找节点，仅限于资金管理系统中账户树使用路径格式为 根节点id|根节点type，父节点id|父节点type，父节点id|父节点type，节点id|节点type
,selectNodeByPath:function(jq,path){
	var evalStr = 'jq';
	$.each(path.split(","),function(i,item){
		var id = item.split("|")[0];
		//循环组装字符串
		evalStr += '.find("div.tree-node[node-id='+id+']").next()';
	});
	evalStr = evalStr.substr(0,evalStr.length - 7);
	var node  = eval(evalStr);
	if(node.length == 0){//当节点不存在时，返回false
		return false;
	}
	var object = _b3(jq[0],node[0]);
	//选中指定节点
	jq.each(function(){_132(this,object.target);});
	return true;
}
};
...
//edit by gm 增加isShadow属性，当isShadow=true时，移动后节点后不删除原有节点
$.fn.tree.defaults={isShadow:false,url:null,method:"post",animate:false,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,lines:false,dnd:false,data:null,loader:function(_15e,_15f,_160){
...
//edit by gm 增加beforeDrop方法（当return=false时，则不进行节点的移动）
...
//edit by gm layout嵌套时，此处可能产生undefined值
if($.data(_17b,"panel") == undefined){
	return false;
}
...
//edit by gm 增加panel的autoWidth属性，允许panel自适应宽度
var p=_17d.parent();
p.addClass("panel-noscroll");
opts.width=p.width();
}
...
//edit by gm 当存在多个子panel时，该方法仅能触发第一个panel的resize方法
//_17d.find(">div.panel-body>div").triggerHandler("_resize");
_17d.find(">div.panel-body>div").each(function(i,item){
$(item).triggerHandler("_resize");
if($(item).find(".easyui-panel").length > 0){
	//当panel中还存在子panel时，此时同样触发resize方法
	$(item).find(".easyui-panel").panel('resize');
}
});
};
...
//edit by gm 增减判断，方式panel被移动到屏幕外
...
//edit by gm 增加panel的autoWidth属性，允许panel自适应宽度
//edit by gm 增加panel resize时触发所有子panel resize方法后，该段代码可注释
...
if(_1a2==true){
//edit by gm 增加判断 当panel被缩起时，如采用了hborder的属性是，会使title模块最下面的横线丢失。此处移除hborder的样式
if(opts.hborder && _1a3.children("div.panel-header").hasClass('panel-header-hborder')){
_1a3.children("div.panel-header").removeClass('panel-header-hborder');
}
...
//edit by gm 增加判断 当panel被缩起时，如采用了hborder的属性是，会使title模块最下面的横线丢失。此处移除hborder的样式
if(opts.hborder){
_1a7.children("div.panel-header").addClass('panel-header-hborder');
}
...
//edit by gm 增加hborder属性。仅在border=false时有效	
if(opts.hborder){
_1b4.addClass("panel-header-hborder");
}else{
...
//edit by gm 弹出框提示框时，默认选中“确认”选项
var btn = $("<a></a>").attr("href","javascript:void(0)").text(_209).css("margin-left",10).bind("click",eval(_208[_209])).appendTo(tb).linkbutton();
if(_209 == $.messager.defaults.ok){
	btn.focus();
}
}
}
...
//当isPercent=true时，宽度为百分比宽度
if(pp.panel("options").isPercent == true){
var height_info = cc.height() * pp.panel("options").height / 100;
pp.panel("options").height = height_info;
pp.panel("options").isPercent = false;//在初始化完成后将isPercent设置为false（防止expand后计算错误）
}
...
//当isPercent=true时，宽度为百分比宽度
if(pp.panel("options").isPercent == true){
var width_info = cc.width() * pp.panel("options").width / 100;
pp.panel("options").width = width_info;
pp.panel("options").isPercent = false;//在初始化完成后将isPercent设置为false（防止expand后计算错误）
}
...
//edit by gm 增加layout中的百分比表示宽度
pp.panel($.extend({},{isPercent:(pp.attr("isPercent")?pp.attr("isPercent")=="true":undefined),width:(pp.length?parseInt(pp[0].style.width)||pp.outerWidth():"auto"),height:(pp.length?parseInt(pp[0].style.height)||pp.outerHeight():"auto"),split:(pp.attr("split")?pp.attr("split")=="true":undefined),doSize:false,cls:("layout-panel layout-panel-"+dir),bodyCls:"layout-body",onOpen:function(){
...
//******************************************
var p=$("<div></div>").appendTo(_2ce).panel({autoWith:false,cls:"layout-expand",title:"&nbsp;",closed:true,doSize:false,tools:[{iconCls:icon,handler:function(){
_2d7(_2ce,_2cf);
return false;
}}]});
...
(function($){
function init(_331){
//edit by gm 更改原先重新创建input的方法为clone页面上的input的方法。此方法的好处在于可直接获取页面上input元素的验证方法。
var _332=$(_331).clone(false,false).removeClass("easyui-searchbox ui-input").addClass("searchbox-text");
$(_331).hide();
...
//edit by gm 取消unbind方法，否则与验证规则冲突
//_33e.unbind(".searchbox").bind("blur.searchbox",function(e){
_33e.bind("blur.searchbox",function(e){
opts.value=$(this).val();
if(opts.value==""){
...
opts.value=$(this).val();
//edit by gm 增加searcher方法中传入控件对象
opts.searcher.call(_33c,opts.value,name,$(_33c));
return false;
}
});
...
$.fn.searchbox.defaults={width:"auto",prompt:"",value:"",menu:null,searcher:function(_34a,name,search_obj){
}};
})(jQuery);
(function($){
function init(_34b){
$(_34b).addClass("validatebox-text");
//逻辑：对于ui-money 增加money验证
if($(_34b).hasClass("ui-money")){
	$(_34b).attr('validType','money');
};
};
function _34c(_34d){
...
function _34f(_350){
//edit by gm 排除combobox关联的text控件，不对其做验证
if($(_350).hasClass("combo-text")){
	return false;
}
var box=$(_350);
...
//edit by gm 此处增加判断，用于解决手都更改easyui-validatebox样式时的验证
if(!box.hasClass("easyui-validatebox")){
	return false;
}
_351.validating=true;
_351.value=undefined;
(function(){
//edit by gm _351.value 可能存在undefined
if(_351.validating){
...
if(box.hasClass("validatebox-invalid") && box.hasClass("easyui-validatebox")){
_353(_350);
}
...
function _353(_354){
var box=$(_354);
//edit by gm 当验证控件为combobox时，value == null时将验证信息显示至text控件位置处
if(box.hasClass('combo-f')){
	box=box.next().find('[linked]');
}
box.addClass("validatebox-invalid");//原_356函数中执行方法，移至此处
var msg=$.data(_354,"validatebox").message;
var tip=$.data(_354,"validatebox").tip;
if(!tip){
//edit by gm 更改验证信息显示方式
tip=$("<div class=\"validatebox-tip\">"+"<span class=\"validatebox-tip-content\">"+"</span>"+"<span class=\"validatebox-tip-pointer\">"+"</span>"+"</div>").appendTo("body");
$.data(_354,"validatebox").tip=tip;
}
tip.find(".validatebox-tip-content").html(msg);
//edit by gm 超出屏幕的验证信息不显示
if(box.offset().left < 0 || box.offset().top < 0){
	tip.hide();
}else{
	tip.css({display:"block",left:box.offset().left+box.outerWidth(),top:box.offset().top}).show();
	window.setTimeout(function(){tip.hide();},2000);//edit by gm 显示2秒后消失
}
};
function _352(_355){
...
function _356(_357){
//edit by gm 初始化时$.data(_357,"validatebox")可能不存在(还有一种情况是,初始化是不含验证,后手动增加验证class).此时返回验证通过
if(!$.data(_357,"validatebox")){
	if(!$(_357).hasClass("easyui-validatebox")){return true;}
	else{
		$.data(_357,"validatebox",{options:$.extend({},$.fn.validatebox.defaults,$.fn.validatebox.parseOptions(_357))});
	}
};
...
var box=$(_357);
var _358=box.val();
//edit by gm 当验证控件为combobox时，value取值方式不同
if(box.hasClass('combo-f')){
	_358=box.next().find('.combo-value').val();
}
function _359(msg){
...
if(opts.required){
if(_358==""){
//box.addClass("validatebox-invalid"); 将该方法移至_353(_357) 中执行
...
var t=$(_361);
//return {required:(t.attr("required")?(t.attr("required")=="required"||t.attr("required")=="true"||t.attr("required")==true):false),validType:(t.attr("validType")||undefined),missingMessage:(t.attr("missingMessage")||undefined),invalidMessage:(t.attr("invalidMessage")||undefined)};
//edit by gm 此处使用notNull=true作为判断非空的条件，在IE10 下面所有input 提交时会自动加上required属性
return {required:(t.attr("notNull")?(t.attr("notNull")=="required"||t.attr("notNull")=="true"||t.attr("notNull")==true):false),validType:(t.attr("validType")||undefined),missingMessage:(t.attr("missingMessage")||undefined),invalidMessage:(t.attr("invalidMessage")||undefined)};
};
...
//edit by gm 删除unbind方法，否则与其他控件并存时验证规则无法生效
//$(_38b).unbind(".numberbox").bind("keypress.numberbox",function(e){
$(_38b).bind("keypress.numberbox",function(e){
if(e.which==45){
return true;
}
if(e.which==46){
...
},setValueNoCall:function(jq,val){
//edit by gm 防止loadJson错误
return jq.each(function(){
_383(this,val);
});
...
//precision:0 -> precision:-1 更改默认不去除小数点后面的数字,默认补0规则为null
$.fn.numberbox.defaults=$.extend({},$.fn.validatebox.defaults,{disabled:false,value:"",min:null,max:null,precision:-1,decimalSeparator:".",groupSeparator:"",prefix:"",suffix:"",formatter:function(_398){
if(!_398){
return _398;
}
_398=_398+"";
...
val=opts.precision > -1?opts.min.toFixed(opts.precision):opts.min;
}else{
if(typeof (opts.max)=="number"&&val>opts.max){
val=opts.precision > -1?opts.max.toFixed(opts.precision):opts.max;
}
}
}
return val;
...
//edit by gm autoRowWidth=true 时，grid宽度自适应
if(opts.autoRowWidth==true){
var p=_402.panel("panel").parent();
opts.width=p.width();
}
...
//edit by gm 解决当控件隐藏再显示时，grid的高度被设置为0
var viewHeight = _408.height() == 0?opts.height:_408.height();
...
//edit by gm IE8 在页面切换时会加载该部分代码，导致$.data(_413,"datagrid") = undefined
if($.data(_413,"datagrid") == undefined){
	return false;
}
...
//edit by gm autoRowWidth=true 时，grid宽度自适应
if(opts.fit==true||_423||opts.autoRowWidth==true){
_3ff(_41d);
setTimeout(function(){
...
//edit by gm 允许在grid上的toolbar中增加下拉框选
if(btn.type == "combobox"){
$("<span>"+btn.label+"</span>").appendTo(tb);
var tool=$("<select id='"+btn.id+"'></select >");
tool.css("float","left").appendTo(tb).combobox($.extend({},btn,{plain:true}));
}else{
...
//edit by gm 增加onClickCell回调返回值，当为return = false时，不选中当前行
return opts.onClickCell.call(_43e,_444,_445,_446);
}).bind("dblclick.datagrid",function(){
...
function _475(_476){
//edit by gm IE8 在页面切换时会加载该部分代码，导致$.data(_413,"datagrid") = undefined
if($.data(_476,"datagrid") == undefined){
	return false;
}
...
//edit by gm 执行loadData时，不加载历史选中行
$.data(_482,"datagrid").selectedRows = new Array();
...
_4b0=true;
_4b1[_4b2]=_4b3;
//edit by gm 增加逻辑，当控件的type=combobox时，读取text的值，format时显示
//if(ed.type == 'combobox'){
//var text = ed.target.combobox('getText');
//row[_4b2 + "_format_show"]= text;
//_4b1[_4b2 + "_format_show"] = text;
//}
}
});
if(_4b0){
...
//edit by gm 当字段存在 x_format_show 格式时，仅显示该格式数据（combobox时会记录text的值）
//if(rows[_57a][_57b  + "_format_show"]){
//cell.html(rows[_57a][_57b  + "_format_show"]);	
//}else{
...
_69b.appendTo("body");
var isAutoWidth = isNaN(opts.width);//用于记录程序是否已设置combobox的宽度
if(isNaN(opts.width)){
...
//edit by gm 去除combo固定设置宽度（仅当设置了width属性时，才允许设置宽度），通过ui-select控制控件宽度.
if(!isAutoWidth){
...
//edit by gm 增加新的样式，允许combo自定义宽度
//var span=$("<span class=\"combo\"></span>").insertAfter(_6a1);
//var _6a2=$("<input type=\"text\" class=\"combo-text\">").appendTo(span);
//$("<span><span class=\"combo-arrow\"></span></span>").appendTo(span);
var span=$("<span class=\"combo ui-select\" style=\"position:relative;\"></span>").insertAfter(_6a1);
var linked = _6a1.id?_6a1.id:"";
var _6a2=$("<input type=\"text\" class=\"combo-text\" style=\"width:100%;padding:1px 0px 0px 0px;\" linked=\""+ linked +"\">").appendTo(span);
$("<span style=\"position:absolute ;right:0px;\"><span class=\"combo-arrow\"></span></span>").appendTo(span);
...
//edit by gm 每次打开combo的下拉框时，需要重新设置下panel的尺寸
_6b1.panel("resize",{width:(opts.panelWidth?opts.panelWidth:_6b0.outerWidth()),height:opts.panelHeight});
...
//edit by gm combo在执行setText方法时不执行验证函数
//_6b7(_6c5,true);
_6b7(_6c5,false);
$.data(_6c5,"combo").previousValue=text;
};
function _6c7(_6c8){
//edit by gm 在页面初始化时执行 datebox('getValue')时，出现异常
if($.data(_6c8,"combo") == undefined){return false;}
var _6c9=[];
...
function _6cb(_6cc,_6cd,isNoCall){
...
if(isNoCall != false){//增加不触发onselect的参数
...
return jq.each(function(){
_6cb(this,_6de);
//edit by gm按钮修改后触发验证事件
if($(this).hasClass('easyui-validatebox') && _6de != ""){
	$(this).validatebox("isValid");
}
});
},setValueNoCall:function(jq,_6de){
//不触发回调事件
return jq.each(function(){
	_6cb(this,_6de,false);
});
},getValue:function(jq){
return _6d1(jq[0]);
...
//edit by gm 更改下拉框选择脚本(增加单引号)，防止存在特殊字符时出错
...
/*
 * edit by gm 该方法移动至_6f5中执行，目的：在setValue时触发select方法
for(var i=0;i<data.length;i++){
...
function _6f5(_6fa,_6fb,_6fc,isNoCall){
...
selectRecordList.push(data[j]);
break;
}
}
vv.push(v);
ss.push(s);
//edit by gm更改下拉框选择脚本(增加单引号)，防止存在特殊字符时出错
...
//edit by gm combobox 执行setValue之后触发onSelect事件
if(!isNoCall){
for(var j=0;j<selectRecordList.length;j++){
opts.onSelect.call(_6fa,selectRecordList[j]);
return;
}
}
};
...
item.attr("value",v);
//edit by gm 当text == “”时，传入&nbsp;防止产生选项div高度不一致的问题
if(opts.formatter){
var htmls = opts.formatter.call(_701,data[i]);
htmls = htmls == "" ? "&nbsp;" : htmls;
item.html(htmls);
}else{
s = s == "" ? "&nbsp;" : s;
...
//edit by gm 增加判断，当下拉框没有设置选中值时，默认选中当前值（在编辑模式下，如果默认选中第一项，则在模糊查询时会永远查询不出结果）(loadData时执行)
if(opts.editable == false){
var defaultValue = $(_701).combobox('getValue');
if((defaultValue == "" || defaultValue == null) && data != null && data.length > 0){
	if(defaultValue != data[0][opts.valueField]){
		//$(_701).combobox('select',data[0][opts.valueField]);
		_6ec(_701,data[0][opts.valueField]);
	}
}
}
};
...
_705(this);
//edit by gm 增加判断，当下拉框没有设置选中值时，默认选中当前值(控件默认加载时执行)
if(_711.options.editable == false){
var defaultValue = $(this).combobox('getValue');
if((defaultValue == "" || defaultValue == null) && _711.options.data != null){
if(_711.options.data.length > 0){
	//$(this).combobox('select',_711.options.data[0][_711.options.valueField]);
	_6ec(this,_711.options.data[0][_711.options.valueField]);
}
}
}
});
...
},setValueNoCall:function(jq,_713){
return jq.each(function(){
_6f5(this,[_713],false,true);//不执行回调函数
});
},clear:function(jq){
return jq.each(function(){
$(this).combo("clear");
...
//edit by gm 解决使用class="combobox"时，在load中由于控件没有初始化导致数据无法加载问题
/*
var ret = jq.each(function(i,item){
loadDataTimer();
function loadDataTimer(){
	if($.data(item,"combobox")==undefined){
		window.setTimeout(function(){loadDataTimer();}, 300);
	}else{
		//edit by gm 增加loading
		var next_span = $(item).next('.combo');
		next_span.css('position','relative');
		$("<div class='combobox-loading'></div>").appendTo(next_span);
		_700(item,data);
		next_span.find('.combobox-loading').remove();
	}
}
});
return ret;
...
//edit by gm 增加loading显示
var next_span = $(this).next('.combo');
$.ajax({type:opts.method,url:opts.url,data:_719,dataType:"json"
...
//edit by gm 增加错误判断，当ajax未连接上时提示出错
if(e.responseText.substr(0,1)){
return alert('error:' + e.responseText);
}
_71b.apply(this,arguments);
//edit by gm 增加当error时弹出提示信息
if(!$.parseJSON(e.responseText)){
return alert(e.responseText);
}
var msg = $.parseJSON(e.responseText).msg;
alert("系统异常：" + msg);
}});
...
//edit by gm 增加判断，ss.join(opts.separator) == "" 时，不执行该方法，否则初始化时会触发验证方法
if(ss.join(opts.separator) != ""){
...
//edit by gm 日期控件当鼠标移开时判断日期格式是否正确，不正确时默认选中当前日期选中的值
$(_751).combo("textbox").blur(function(){
	var box_text = $(this).val();
	if(box_text == ""){return false;}//允许日期控件为空
	var date_str = _752.calendar.calendar('options').current.format('yyyy-MM-dd');
	if(box_text != date_str){
		$(this).val(date_str);
		if(opts.onChange){opts.onChange(date_str);}
		opts.onSelect.call(_751,date_str);
	}
});
function _753(){
...
//设置日历默认值
/*
if($(_751).datebox('getValue') == ""){
	$(_751).datebox('setValue',new Date().format("yyyy-MM-dd"));
}
*/
};
...
},setValueNoCall:function(jq,_764){//edit by gm 增加该方法用于统一赋值
return jq.each(function(){
	//edit by gm 防止datebox未加载完成即开始赋值
//setValueCall(this);
_759(this,_764);
});
...
//edit by gm 增加数字规则判断
//"^-?\\d{0,2}\\.\\d{0,3}$"  允许为负，两位整数，3位小数
$.extend($.fn.validatebox.defaults.rules, {   
numLength: {
validator: function(value, param){
	var integer,Decimal,types,reg;
	var paramLen = param.length;
	if(paramLen > 0){integer = param[0];}
	if(paramLen > 1){Decimal = param[1];}
	if(paramLen > 2){types = param[2];}
	
	if(types == "pn"){reg = "^\\";} //正数
	else if(types == "pn"){reg = "^-\\";}//负数
	else {reg = "^-?\\";}
	reg += "d{0,"+integer+"}";//整数部分
	if(Decimal){
		//增加逻辑,当验证规则为Decimal时，允许界面仅输入整数部分，不输入小数值
		var regOther = reg + "$";
		reg += "\\.\\d{0,"+Decimal+"}";
		reg += "$";
		return new RegExp(reg).test(value) || new RegExp(regOther).test(value) ;
	}else{
		reg += "$";
...
message: function(param){
	if(!param[1] || param[1] == "0"){
		return '请输入有效的数字,整数部分长度不允许超过{0}.';
	}
	else{
		return "请输入有效的数字,整数部分长度不允许超过{0},小数部分长度不允许超过{1}.";
	}
}
},
strLength:{
	validator: function(value, param){
		...
		var paramLen = param.length;
		//如果仅一个数字时，要求字符串长度必须等于该数字
		if(paramLen == 1 && len != param[0]){
			return false;
		}else if(len < param[0] || len > param[1]){
			return false;
		}
		return true;
	},
...
$.extend($.fn.validatebox.defaults.rules, {  
    equals: {
        ...
        validator: function(value){
            if(isNaN(CommonUtil.stringUtil.moneyRevert(value))){return false;}
            if(parseFloat(CommonUtil.stringUtil.moneyRevert(value))<0){return false;}
            return true;
        },  
        message: '金额字段必须为大于0的数字.' 
    }
});  


//线程暂停函数（消耗CPU尽量少用，在datebox取值与设值时使用，防止datebox未加载完成就开始取值）
function easyUI_sleep(milliSeconds){
	var startTime = new Date().getTime();
	while (new Date().getTime() < startTime + milliSeconds);
}

//扩展combo，增加checkbox下拉列表。（不改变原combo控件，所有函数依然使用combo中的属性）
(function($){
	$.fn.combocheck=function(param,val){
		//当传入setValue方法时
		if(typeof param == "string"){
			var fun = $.fn.combocheck.methods[param];
			if(fun){
				return fun(this,val);
			}else{
				return this.combo(param,val);
			}
		}
		param = $.extend({},$.fn.combocheck.defaults,param);
		
		var control_info = this;
		control_info.combo(param);
		//control_info.combo({editable:false});
		createPanel(control_info);
		return this;
	};
	
	function createPanel(control_info){
		var param = control_info.combo("options");
		control_info.combo('panel').empty();
		var panel = $('<div id="combo_check_panel"></div>');
		//逻辑:添加“全选”项
		var all_check = $('<div class="combobox-item" style="background-color:#FAFAFA"><input style="vertical-align:middle" type="checkbox" /><label style="vertical-align:middle">全部</label></div>');
		all_check.click(function(){
			//设置全选方法
			if(all_check.find(':checkbox').attr("checked") == "checked"){
				all_check.parent().find(":checkbox").attr("checked","checked");
			}else{
				all_check.parent().find(":checkbox").removeAttr("checked");
			}
			setValueOfCombo();
			if(param.onCheck){
				param.onCheck(param.data);
			}
		});
		all_check.appendTo(panel);
		
		//逻辑:添加子选项
		if(param.data){
			var valueField = "value";
			var textField = "text";
			if(param.valueField){valueField = param.valueField;}
			if(param.textField){textField = param.textField;}
			$.each(param.data,function(i,item){
				var info_check = $('<div class="combobox-item"><input style="vertical-align:middle" type="checkbox" value="'+item[valueField]+'" text="'+item[textField]+'" /><label style="vertical-align:middle">'+item[textField]+'</label></div>');
				...
				info_check.appendTo(panel);
			});
		}
		
		//函数：combo赋值方法
		function setValueOfCombo(){
			control_info.combo('setValue', "").combo('setText', "");
			var values = "";
			var texts = "";
			panel.find(':checkbox[text][checked="checked"]').each(function(i,item){
				values += $(item).attr('value') + ",";
				texts += $(item).attr('text') + ",";
			});
			if(texts != ""){texts = texts.substr(0,texts.length -1);values = values.substr(0,values.length -1);}
			control_info.combo('setValue', values).combo('setText',texts);
		}
		
		//逻辑:panel添加至combo中
		panel.appendTo(control_info.combo('panel'));
		
		//逻辑：对combocheckbox增加特定样式
		control_info.addClass('easyui-combocheck');
	}
	
	$.fn.combocheck.defaults = {
			editable:false,
			data:null,
	...
	$.fn.combocheck.methods = {
		setValue:function(jq,val){
			...
			var text = "";
			if(CommonUtil.stringUtil.isNull(val)){return false;}
			$.each(val.split(','),function(i,item){
				var check = panel.find(':checkbox[text][value="'+item+'"]');
				if(check.length > 0){
					check.attr('checked','checked');
					text += check.attr('text') + ",";
				}
			});
			if(text != ""){text = text.substr(0,text.length -1);}
			return $(jq).combo('setValue', val).combo('setText',text);
		},
		setValueNoCall:function(jq,val){
			var panel = $(jq).combo('panel');
			var text = "";
			...
			if(text != ""){text = text.substr(0,text.length -1);}
			$(jq).combo('textbox').val(text);
			$.data(jq[0],"combo").combo.find("input.combo-value").val(val);
		},
		getValue:function(jq){
			return $(jq).combo('getValue');
		},
		getSplitValue:function(jq){
			var combbValue = $(jq).combo('getValue');
			return "'" + combbValue.replaceAll(",","','") + "'";
		},
		loadData:function(jq,val){
			$(jq).combo("options").data = val;
			createPanel($(jq));
		}
	};
})(jQuery);
...
(function($){
	var beforeValue = "";
	$.fn.fuzzycombo=function(param,val){
		//当传入setValue方法时
		if(typeof param == "string"){
			var fun = $.fn.fuzzycombo.methods[param];
			if(fun){
				return fun(this,val);
			}else{
				return this.combobox(param,val);
			}
		}
		param = $.extend({},$.fn.fuzzycombo.defaults,param,{textField:param.Field,valueField:param.Field});
		var control_info = this;
		control_info.combobox(param);
		control_info.parent().find('.combo-arrow').addClass('searchbox-button').click(function(){
			//解决：用户直接选中默认的下拉数据时，不触发模糊查询动作.此时虽然选中查询结果，但依然存在其他选项
			var $panel  = control_info.combobox('panel');
			if($panel.find('.combobox-item-selected').length > 0){
				$panel.find('.combobox-item:not(.combobox-item-selected)').remove();
			}
		});
		control_info.combobox('textbox').keyup(function(){
			if(param.timeoutCall != null){
				clearTimeout(param.timeoutCall);
			}
			var txt = control_info.combobox('getText');
			//模糊查询连续两次输入相同值时则不触发模糊查询动作
			if(txt == beforeValue){
				return false;
			}
			//当txt的值为空时，需要触发一次onSelect的值
			if(txt == ""){
				param.onSelect();
				control_info.combobox('setValue',"");
			}
			param.timeoutCall = window.setTimeout(function(){loadData(control_info,txt);},1000);
		}).focus(function(){
			//在执行模糊查询前，将控件中更改前的值记录下来
			beforeValue = control_info.combobox('getValue');
		}).blur(function(){
			//鼠标移开之后延迟500毫秒内判断是否选中，方式用户选中下拉框时就触发该方法
			window.setTimeout(function(){
				if(CommonUtil.stringUtil.isNull($.data(control_info,"combobox"))){return false;}
				var val = control_info.combobox('getValue');
				var record = getComboxRecord(control_info,val);
				if(record != null){return false;}
				//查询无数据时，则返回上次结果(仅当鼠标移开时触发)
				record = getComboxRecord(control_info,beforeValue);
				if(!CommonUtil.stringUtil.isNull(record)){
					control_info.combobox('setText',record[param.Field]);
				}else{
					control_info.combobox('setText',"");
					param.onSelect();
				}
			},500);
		});
		//逻辑：对combocheckbox增加特定样式
		control_info.addClass('easyui-fuzzycombo');
		//初始化控件时执行一次加载程序
		loadData(control_info,"");
		return this;
	};
	
	function loadData(control_info,value){
		var $textbox = control_info.combobox("textbox");
		$textbox.attr("readonly","readonly");
		var param = control_info.combobox("options");
		if(CommonUtil.stringUtil.isNull(param.hasCashAcc)){param.hasCashAcc = false;}
		CommonUtil.ajax({
			url:param.urls,
			data:{"text":value,"limit":20,"hasCashAcc":param.hasCashAcc},
			callback:function(data) {
				$textbox.removeAttr("readonly");
				if(data.length > 0){
					control_info.combobox('loadData',data);
				}
			}
		});
	};
	...
	//读取combox的选中记录
	function getComboxRecord(control_info,val){
		var dataList = control_info.combobox('getData');
		var param = control_info.combobox("options");
		for(var i=0;i<dataList.length;i++){
			if(dataList[i][param.Field] == val){
				return dataList[i];
			}
		}
		return null;
	...
	$.fn.fuzzycombo.defaults = {
		editable:true,
		timeoutCall:null,//用于定时执行模糊查询
		onSelect:function(record){}
	}; 
	
	$.fn.fuzzycombo.methods = {
		setValue:function(jq,val){
			var param = $(jq).combobox("options");
			if(CommonUtil.stringUtil.isNull(param.hasCashAcc)){param.hasCashAcc = false;}
			CommonUtil.ajax({
				url:param.urls,
				data:{"text":val,"limit":20,"hasCashAcc":param.hasCashAcc},
				callback : function(data) {
					if(CommonUtil.stringUtil.isNull(data) || data.length != 1){
						$.messager.alert("提示信息","未能正确匹配模糊查询的值.");
						return false;
					}
					$(jq).combobox("setValue",data[0][param.Field]);
				}
			});
		},
		setValueNoCall:function(jq,val){
			var param = $(jq).combobox("options");
			if(CommonUtil.stringUtil.isNull(param.hasCashAcc)){param.hasCashAcc = false;}
			$(jq).combobox("setValueNoCall",val);
			/*不用改方法，防止出现传入user同时匹配上user 和 user_name的情况
			CommonUtil.ajax({
				url:param.urls,
				data:{"text":val,"limit":20,"hasCashAcc":param.hasCashAcc},
				callback : function(data) {
					...
					$(jq).combobox("setValueNoCall",data[0][param.Field]);
				}
			});
			*/
		},
